...
def train_agent(
    ...
    epsilon_decay_frames: int = 1000000,
    batch_size: int = 32,
    ...
    # Prepare observation into MLX ready format.
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps_per_episode):
            ...
            # Train model
            if frame_count > random_frames and frame_count % train_freq == 0 and len(replay_buffer) > batch_size:
            ...
            if frame_count % target_update_freq == 0:
                target_model.update(model.trainable_parameters())
                print(f"Episode {episode+1}/{num_episodes}, Frame {frame_count}, Epsilon {epsilon:.4f}")
        ...
        print(f"Episode {episode+1}/{num_episodes}, Reward: {episode_reward}, Avg Reward: {avg_reward:.2f}, Epsilon: {epsilon:.4f}")
    ...
    # Close environment
    env.close()
    return model, episode_rewards, running_rewards
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Deep Q-Network implementation with MLX')
    ...
    parser.add_argument('--save', type=str, default='./models', help='Path to save model weights')
    
    args = parser.parse_args()
    
    if args.mode == 'train':
        model, rewards, avg_rewards = train_agent(
            env_name=args.env, 
        ...
        model = DQN(num_actions=4)  # Assuming standard Atari with 4 actions
